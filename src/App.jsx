...
function TaskPopup({ taskPopup, tasks, schedule, getTaskTime, toggleComplete, removeFromDay, assignToDay, togglePin, toggleLock, startEditing, setTaskPopup, getCat, getDurType, getDurationLabel, fmtMins, getDeadlineStatus, saveWeekSchedule, weekOffset, showToast, setTimePicker }) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "#00000033", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setTaskPopup(null)}>
      <div style={{ background: "#f4f0e8", border: `2px solid ${cat.color}`, borderRadius: 14, padding: 20, width: 300, maxWidth: "90vw", boxShadow: "0 8px 32px #00000022" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <button
            onClick={() => { toggleLock(task.id); setTaskPopup(null); showToast(task.locked ? "🔓 Unlocked" : "🔒 Locked — Plan My Week won't move this"); }}
            style={{ background: task.locked ? "#e8e4f0" : "none", border: `1px solid ${task.locked ? "#b8a4c8" : "#d6d0c8"}`, color: task.locked ? "#8a7ea8" : "#6c6255", padding: "7px 12px", borderRadius: 7, fontSize: 12, cursor: "pointer" }}
...
export default function App() {
  ...
  // Re-flow all task times on a day sequentially
  const reFlowDayTimes = (day, scheduleSnapshot, anchorTaskId) => {
    ...
    const getTaskDur = (t) => t.durationType === "single" ? (t.estimatedMins || 30)
      ...
      : (t.sessionMins || 30);
    // Sort: anchor task first (it sets the starting point), then others by existing time
    const sorted = [...entries].sort((a, b) => {
      const aId = typeof a === "object" ? a.id : a;
      const bId = typeof b === "object" ? b.id : b;
      if (aId === anchorTaskId) return -1;
      if (bId === anchorTaskId) return 1;
    ...
    // For anchor task — start from its set time, not work start
    const anchorEntry = sorted.find(e => (typeof e === "object" ? e.id : e) === anchorTaskId);
    const anchorTime = anchorEntry?.time;
    let cursor = anchorTime
      ? parseInt(anchorTime.split(":")[0]) * 60 + parseInt(anchorTime.split(":")[1] || 0)
      : workStart;
    const newEntries = sorted.map(e => {
      const id = typeof e === "object" ? e.id : e;
      const task = tasks.find(t => t.id === id);
      if (!task || task.durationType === "ongoing") return e;
      const dur = getTaskDur(task);
      // If this is the anchor task, keep its time and just advance cursor
      if (id === anchorTaskId && anchorTime) {
        cursor += dur;
        return { id, time: anchorTime };
      }
      let placed = false;
      let attempts = 0;
  ...
  const setTaskTime = (day, taskId, time) => {
    // First update the time for this task
    let updated = { ...activeSchedule, [day]: (activeSchedule[day] || []).map((e) => { const id = typeof e === "object" ? e.id : e; return id === taskId ? { id, time } : e; }) };
    // Then re-flow the whole day so tasks below shift accordingly
    if (time) {
      updated = reFlowDayTimes(day, updated, taskId);
    }
    saveWeekSchedule(weekOffset, updated);
  };
  ...
  // Lock/unlock a task — locked tasks are never moved by Plan My Week
  const toggleLock = (taskId) => {
    setTasks(p => p.map(t => t.id === taskId ? { ...t, locked: !t.locked } : t));
  };
  ...
  const planMyWeek = () => {
    ...
    const isAlreadyPlanned = (taskId, durationType) => {
      if (durationType === "ongoing") return false;
      if (thisWeekScheduledIds.has(taskId)) return true;
      if (durationType === "single" && otherWeekIds.has(taskId)) return true;
      return false;
    };

    const ongoingTasks = tasks.filter((t) => t.durationType === "ongoing" && !t.completed && !t.locked);
    const multidayTasks = tasks.filter((t) => t.durationType === "multiday" && !t.completed && !t.locked && !thisWeekScheduledIds.has(t.id));
    const singleTasks = tasks.filter((t) => t.durationType === "single" && !t.completed && !t.locked && !isAlreadyPlanned(t.id, "single"));
    ...
    const skippedCount = tasks.filter(t => t.durationType === "single" && !t.completed && (isAlreadyPlanned(t.id, "single") || t.locked)).length;
    const lockedCount = tasks.filter(t => t.locked && !t.completed).length;
    const tips = `Planning ${plannedCount} task${plannedCount !== 1 ? "s" : ""} across ${remainingDays} day${remainingDays !== 1 ? "s" : ""} — ~${Math.round(totalMins / 60 * 10) / 10} hours total.${skippedCount > 0 ? ` ${skippedCount} task${skippedCount !== 1 ? "s" : ""} skipped (already scheduled or locked).` : ""}${lockedCount > 0 ? ` 🔒 ${lockedCount} locked task${lockedCount !== 1 ? "s" : ""} left in place.` : ""} ${fixedBlocks.length > 0 ? "Scheduled around your fixed commitments." : "Add fixed blocks in Fixed Schedule so future plans can work around them."}`;
          ...
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              ...
              <button onClick={planMyWeek} disabled={aiPlanning || tasks.length === 0} style={{ background: aiPlanning ? "#e9e4db" : "linear-gradient(135deg,#8aa898,#a8c4b0)", border: "none", color: "#fff", padding: "7px 14px", borderRadius: 6, fontSize: 12, cursor: tasks.length === 0 ? "not-allowed" : "pointer", opacity: tasks.length === 0 ? 0.4 : 1 }}>
                {aiPlanning ? "Planning…" : unscheduledTasks.length > 0 ? `✦ Plan ${unscheduledTasks.length} Unscheduled` : "✦ Plan My Week"}
              </button>
            </div>
            ...
            {subView === "schedule" && (
              <>
                {/* ── Unscheduled Tray ── */}
                <div style={{ marginBottom: 12, background: "#ece7df", border: "1px solid #d6d0c8", borderRadius: 9, overflow: "hidden" }}>
                  {/* Tray header */}
                  <div
                    onClick={() => setShowUnscheduledTray(s => !s)}
                    style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 12px", cursor: "pointer" }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      <span style={{ fontSize: 10, color: "#8a7e76", letterSpacing: "0.15em", textTransform: "uppercase" }}>Unscheduled</span>
                      {unscheduledTasks.length > 0 && (
                        <span style={{ background: "#a8c4b0", color: "#fff", borderRadius: 10, fontSize: 9, padding: "1px 6px" }}>{unscheduledTasks.length}</span>
                      )}
                    </div>
                    <span style={{ fontSize: 10, color: "#a09890" }}>{showUnscheduledTray ? "▲" : "▼"}</span>
                  </div>
                  ...
                  {showUnscheduledTray && (
                    <div
                      ...
                      onDrop={() => {
                        if (dragTask && dragFromDay) {
                          // Move back to unscheduled — remove from day
                          removeFromDay(dragTask, dragFromDay);
                          setDragTask(null); setDragFromDay(null); setDragOver(null);
                          showToast("Moved back to unscheduled");
                        }
                      }}
                      style={{ padding: "6px 12px 10px", minHeight: 44, borderTop: "1px solid #d6d0c8", background: dragOver === "unscheduled" ? "#e0dbd4" : "#ece7df", transition: "background 0.12s" }}
                    >
                      {unscheduledTasks.length === 0 && (
                        <div style={{ fontSize: 10, color: "#b0a898", textAlign: "center", padding: "8px 0" }}>
                          {dragTask && dragFromDay ? "Drop here to unschedule" : "All tasks scheduled ✓"}
                        </div>
                      )}
                        ...
                        {unscheduledTasks.map((task) => {
                          ...
                          return (
                            <div
                              key={task.id}
                              draggable
                              onDragStart={() => { setDragTask(task.id); setDragFromDay(null); }}
                              onDragEnd={() => { setDragTask(null); setDragFromDay(null); setDragOver(null); }}
                              style={{ background: "#f4f0e8", border: `1px solid ${cat.color}44`, borderLeft: `3px solid ${cat.color}`, borderRadius: 5, padding: "4px 9px", cursor: "grab", display: "flex", alignItems: "center", gap: 5, fontSize: 12, opacity: dragTask === task.id ? 0.4 : 1 }}
                            >
                              <CatIcon icon={cat.icon} color={cat.color} size={10} />
                              {task.priority && <span style={{ fontSize: 9 }}>❗</span>}
                              {task.locked && <span style={{ fontSize: 9 }}>🔒</span>}
                              <span style={{ color: "#4c4235", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{task.title}</span>
                            </div>
                          );
                        })}
                      </div>
                      {dragTask && dragFromDay && (
                        <div style={{ fontSize: 9, color: "#a8c4b0", textAlign: "center", marginTop: 6 }}>↑ Drop here to move back to unscheduled</div>
                      )}
                    </div>
                  )}
                </div>
                              ...
                              <div style={{ display: "flex", alignItems: "flex-start", gap: 5 }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <div style={{ display: "flex", alignItems: "center", gap: 3, flexWrap: "wrap", marginBottom: 1 }}>
                                    ...
                                    {task.locked && <span style={{ fontSize: 8, flexShrink: 0 }}>🔒</span>}
