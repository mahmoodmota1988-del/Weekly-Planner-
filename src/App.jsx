  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f4f0e8", color: "#3c3226", fontFamily: "Georgia, serif" }}>
      ...
      <main style={{ padding: "16px 18px", paddingBottom: 80 }}>
        ...
        {/* ── FOCUS MODE OVERLAY ── */}
        {focusMode && (
          <div style={{ position: "fixed", inset: 0, background: "#f4f0e8", zIndex: 400, overflowY: "auto", padding: 24 }}>
            <div style={{ maxWidth: 480, margin: "0 auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <div>
                  <div style={{ fontSize: 10, color: "#8a7e76", letterSpacing: "0.2em", textTransform: "uppercase" }}>Focus Mode</div>
                  <div style={{ fontSize: 20, color: "#3c3226" }}>Today — {todayName}</div>
                </div>
                <button onClick={() => setFocusMode(false)} style={{ background: "none", border: "1px solid #d6d0c8", color: "#8a7e76", borderRadius: 7, padding: "7px 14px", fontSize: 12, cursor: "pointer" }}>Exit Focus</button>
              </div>
              {/* Today's tasks only */}
              {(() => {
                const todayIds = getScheduleIds(todayName);
                const todayTasks = todayIds.map(id => tasks.find(t => t.id === id)).filter(Boolean);
                const pinnedTasks = todayTasks.filter(t => t.pinned);
                const unpinnedTasks = todayTasks.filter(t => !t.pinned);
                const sorted = [...pinnedTasks, ...unpinnedTasks];
                if (!sorted.length) return <div style={{ textAlign: "center", padding: "40px 0", color: "#a09890", fontSize: 14 }}>No tasks scheduled for today.<br/><span style={{ fontSize: 12 }}>Go to Week View to add some!</span></div>;
                return sorted.map(task => {
                  const cat = getCat(task.category);
                  const taskTime = getTaskTime(todayName, task.id);
                  return (
                    <div key={task.id} style={{ background: task.completed ? "#ece7df" : "#fff", border: `1px solid ${task.completed ? "#d6d0c8" : cat.color + "44"}`, borderLeft: `4px solid ${task.completed ? "#d6d0c8" : cat.color}`, borderRadius: 10, padding: "14px 16px", marginBottom: 10, display: "flex", alignItems: "flex-start", gap: 12 }}>
                      <input type="checkbox" checked={task.completed} onChange={() => toggleComplete(task.id)} style={{ accentColor: cat.color, width: 18, height: 18, marginTop: 2, cursor: "pointer", flexShrink: 0 }} />
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                          {task.pinned && <span style={{ fontSize: 11 }}>📌</span>}
                          {task.priority && <span style={{ fontSize: 12 }}>❗</span>}
                        ...
                        <div style={{ fontSize: 11, color: "#8a7e76" }}>{getCatLabel(task.category)} · {getDurationLabel(task)}{taskTime ? ` · 🕐 ${taskTime}` : ""}</div>
                        {task.notes && <div style={{ fontSize: 11, color: "#a09890", marginTop: 4 }}>{task.notes}</div>}
                      </div>
                    </div>
                  );
                });
              })()}
              <button onClick={() => { setShowDayReview(true); setFocusMode(false); }} style={{ background: "linear-gradient(135deg,#8aa898,#a8c4b0)", border: "none", color: "#fff", borderRadius: 8, padding: "12px 20px", fontSize: 13, cursor: "pointer", width: "100%", marginTop: 10 }}>
                End of Day Review →
              </button>
            </div>
          </div>
