...
const defaultTask = {
  ...
  recurEnd: "", // date string YYYY-MM-DD or empty = forever
};
const defaultFixed = {
...
export default function App() {
  ...
  const onDrop = (toDay) => {
    ...
    if (dragFromDay && dragFromDay !== toDay) {
      ...
      // Find a free slot on the new day based on existing tasks there
      const toEntries = updated[toDay] || [];
      const toIds = toEntries.map((e) => (typeof e === "object" ? e.id : e));
      if (!toIds.includes(dragTask)) {
        // Build used slots from existing tasks on toDay
        const usedSlots = toEntries.map(e => {
          const id = typeof e === "object" ? e.id : e;
          ...
          const dur = t.durationType === "single" ? (t.estimatedMins || 30)
            : t.durationType === "multiday" ? (t.minsPerSession || 30)
            : (t.sessionMins || 30);
          return { start, end: start + dur };
        }).filter(Boolean);
        // Add fixed block slots
        fixedBlocks.filter(b => b.day === toDay).forEach(b => {
          usedSlots.push({ start: b.startHour * 60 - 15, end: b.endHour * 60 + 15 });
        });
        const workStart = workHours[toDay]?.start ?? 9;
        const slot = findSlot(toDay, durMins, [], workStart, usedSlots);
        updated[toDay] = [...toEntries, { id: dragTask, time: slot.time24 }];
        saveWeekSchedule(weekOffset, updated);
        if (slot.time24) {
          showToast(`Moved to ${toDay} at ${slot.time24}`);
        } else {
      ...
      if (!ids.includes(dragTask)) {
        ...
        const workStart = workHours[toDay]?.start ?? 9;
        const slot = findSlot(toDay, durMins, [], workStart, usedSlots);
        const updated = { ...activeSchedule, [toDay]: [...entries, { id: dragTask, time: slot.time24 }] };
        saveWeekSchedule(weekOffset, updated);
        if (!slot.time24) setTimePicker({ taskId: dragTask, day: toDay });
      }
    }
