...
function TaskPopup({ taskPopup, tasks, schedule, getTaskTime, toggleComplete, removeFromDay, assignToDay, togglePin, startEditing, setTaskPopup, getCat, getDurType, getDurationLabel, fmtMins, getDeadlineStatus }) {
      ...
      <div style={{ background: "#f4f0e8", border: `2px solid ${cat.color}`, borderRadius: 14, padding: 20, width: 300, maxWidth: "90vw", boxShadow: "0 8px 32px #00000022" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 16, color: "#3c3226", lineHeight: 1.3, textDecoration: task.completed ? "line-through" : "none", color: task.completed ? "#a09890" : "#3c3226" }}>{task.title}</div>
          </div>
        ...
        <div style={{ background: "#ece7df", borderRadius: 8, padding: "10px 12px", marginBottom: 12, display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          {taskTime && (
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <span style={{ fontSize: 11, color: "#82b99a" }}>🕐 {taskTime}</span>
            </div>
          )}
          <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              {DAYS.map((d) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (!isCurrentDay) {
                        removeFromDay(task.id, taskPopup.day);
                        assignToDay(task.id, d);
                        setTaskPopup(null);
                      }
                    }}
...
export default function App() {
  ...
  const onDrop = (toDay) => {
    ...
    if (dragFromDay && dragFromDay !== toDay) {
      ...
      if (!toIds.includes(dragTask)) updated[toDay] = [...toEntries, { id: dragTask, time: existingTime }];
      saveWeekSchedule(weekOffset, updated);
      setTimePicker({ taskId: dragTask, day: toDay });
    } else if (!dragFromDay) {
      const entries = activeSchedule[toDay] || [];
      const ids = entries.map((e) => (typeof e === "object" ? e.id : e));
      if (!ids.includes(dragTask)) {
        const updated = { ...activeSchedule, [toDay]: [...entries, { id: dragTask, time: null }] };
        saveWeekSchedule(weekOffset, updated);
      }
      setTimePicker({ taskId: dragTask, day: toDay });
    }
  ...
  const planMyWeek = () => {
    ...
    // Track used time slots per day to avoid double booking
    const dayUsedSlots = {};
    DAYS.forEach(d => { dayUsedSlots[d] = []; });
    ...
    [...multidayTasks].sort(sortByCat).forEach((task) => {
      ...
      for (const { d } of sortedDays) {
        ...
        if (dayLoad[d] + (task.minsPerSession || 30) <= dayFreeMinutes[d]) {
          ...
          if (slot.time24) dayUsedSlots[d].push(slot.time24);
    ...
    [...singleTasks].sort(sortByCat).forEach((task) => {
      ...
      for (const day of enabledPref) {
        if (dayLoad[day] + mins <= dayFreeMinutes[day]) {
          ...
          if (slot.time24) dayUsedSlots[day].push(slot.time24);
          dayLoad[day] += mins;
          placed = true;
          break;
        }
  ...
  const findSlot = (day, durationMins, fixedSlots, workStart = 9, usedSlots = []) => {
    ...
    // Also block already-used slots
    const used = usedSlots.map((t) => {
    ...
    while (cursor + durationMins <= busyEnd) {
      ...
      const blockedByUsed = used.some((u) => cursor < u.end && end > u.start);
      if (!blockedByFixed && !blockedByUsed) {
        const h = Math.floor(cursor / 60);
        const m = cursor % 60;
        const endH = Math.floor(end / 60);
        const endM = end % 60;
        // Return in 24hr format for storage AND display format
        const time24 = `${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}`;
        const timeDisplay = `${fmtHour(h)}${m > 0 ? ` ${m}` : ""} – ${fmtHour(endH)}${endM > 0 ? ` ${endM}` : ""}`;
        return { time24, display: timeDisplay };
      }
      cursor += 30;
    }
    return { time24: null, display: "Flexible" };
  };
    ...
    <div style={{ minHeight: "100vh", background: "#f4f0e8", color: "#3c3226", fontFamily: "Georgia, serif" }}>
      ...
      <main style={{ padding: "16px 18px", paddingBottom: 80 }}>
        ...
        {focusMode && (() => {
          ...
          const isToday = activeFocusDay === todayName;
          const dayIds = getScheduleIds(activeFocusDay);
          ...
          const pinnedTasks = dayTasks.filter(t => t.pinned);
          const unpinnedTasks = dayTasks.filter(t => !t.pinned);
          const sorted = [...pinnedTasks, ...unpinnedTasks].sort((a, b) => {
            const ta = getTaskTime(activeFocusDay, a.id) || "99:99";
            const tb = getTaskTime(activeFocusDay, b.id) || "99:99";
            return ta.localeCompare(tb);
          });
          const completedCount = sorted.filter(t => t.completed).length;

          return (
            <div style={{ position: "fixed", inset: 0, background: "#f4f0e8", zIndex: 400, overflowY: "auto", padding: "20px 18px" }}>
              <div style={{ maxWidth: 480, margin: "0 auto" }}>

                {/* Header */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ fontSize: 10, color: "#8a7e76", letterSpacing: "0.2em", textTransform: "uppercase" }}>Focus Mode</div>
                  <button onClick={() => { setFocusMode(false); setFocusDay(null); }} style={{ background: "none", border: "1px solid #d6d0c8", color: "#8a7e76", borderRadius: 7, padding: "6px 14px", fontSize: 12, cursor: "pointer" }}>Exit</button>
                </div>

                {/* Day navigator */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                  <button
                    onClick={() => setFocusDay(prevDay)}
                    disabled={!prevDay}
                    style={{ background: "none", border: "1px solid #d6d0c8", color: prevDay ? "#6c6255" : "#d6d0c8", borderRadius: 7, padding: "8px 14px", fontSize: 13, cursor: prevDay ? "pointer" : "not-allowed" }}
                  >← {prevDay ? prevDay.slice(0, 3) : ""}</button>

                  <div style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 22, color: "#3c3226", fontWeight: "normal" }}>{activeFocusDay}</div>
                    <div style={{ fontSize: 11, color: isToday ? "#82b99a" : "#a09890" }}>
                      {isToday ? "Today" : `${getDayDate(activeFocusDay, weekOffset).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}`}
                      {sorted.length > 0 && ` · ${completedCount}/${sorted.length} done`}
                    </div>
                  </div>

                  <button
                    onClick={() => setFocusDay(nextDay)}
                    disabled={!nextDay}
                    style={{ background: "none", border: "1px solid #d6d0c8", color: nextDay ? "#6c6255" : "#d6d0c8", borderRadius: 7, padding: "8px 14px", fontSize: 13, cursor: nextDay ? "pointer" : "not-allowed" }}
                  >{nextDay ? nextDay.slice(0, 3) : ""} →</button>
                </div>

                {/* Day selector pills */}
                <div style={{ display: "flex", gap: 5, justifyContent: "center", marginBottom: 20 }}>
                  {DAYS.map((d, i) => {
                    const dIds = getScheduleIds(d);
                    ...
                    const isDToday = d === todayName;
                    return (
                      <button key={d} onClick={() => setFocusDay(d)} style={{ background: isActive ? "#8aa898" : "none", border: `1px solid ${isActive ? "#8aa898" : "#d6d0c8"}`, color: isActive ? "#fff" : isDToday ? "#82b99a" : "#a09890", borderRadius: 5, padding: "4px 7px", fontSize: 10, cursor: "pointer", position: "relative", fontWeight: isDToday ? "bold" : "normal" }}>
                        {d.slice(0, 3)}
                ...
                {sorted.length === 0 && dayFixed.length === 0 && (
                  <div style={{ textAlign: "center", padding: "40px 0", color: "#a09890", fontSize: 14 }}>
                    Nothing scheduled for {activeFocusDay}.<br />
                    <span style={{ fontSize: 12 }}>Go to Week View to add tasks!</span>
                  </div>
                )}

                {sorted.map(task => {
                  const cat = getCat(task.category);
                  const taskTime = getTaskTime(activeFocusDay, task.id);
                  ...
                  return (
                    <div key={task.id} style={{ background: task.completed ? "#ece7df" : "#fff", border: `1px solid ${task.completed ? "#d6d0c8" : cat.color + "44"}`, borderLeft: `4px solid ${task.completed ? "#d6d0c8" : cat.color}`, borderRadius: 10, padding: "14px 16px", marginBottom: 8, display: "flex", alignItems: "flex-start", gap: 12 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "#8a7e76", display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                {/* End of day review — only show for today */}
                {isToday && sorted.length > 0 && (
                  <button onClick={() => { setShowDayReview(true); setFocusMode(false); setFocusDay(null); }} style={{ background: "linear-gradient(135deg,#8aa898,#a8c4b0)", border: "none", color: "#fff", borderRadius: 8, padding: "13px 20px", fontSize: 13, cursor: "pointer", width: "100%", marginTop: 8 }}>
                    End of Day Review →
                  </button>
                )}
              </div>
            ...
            <div style={{ background: "#ece7df", borderRadius: 10, padding: 16, marginBottom: 14 }}>
              ...
              {editingCatLabels && CATEGORIES.map(cat => (
                <div key={cat.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  <span style={{ color: cat.color, fontSize: 14, width: 20 }}>{cat.icon}</span>
                  <input
                            ...
                            <div
                              ...
                              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 4 }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 4, flex: 1, minWidth: 0 }}>
                                  ...
                                  {task.pinned && <span style={{ fontSize: 9, flexShrink: 0 }}>📌</span>}
                                  {task.priority && <span style={{ fontSize: 10, flexShrink: 0 }}>❗</span>}
                                  {task.deadline && !task.completed && getDeadlineStatus(task.deadline)?.urgent && <span style={{ fontSize: 9, color: getDeadlineStatus(task.deadline).color, flexShrink: 0 }}>📅</span>}
                                  {overdue && <span style={{ fontSize: 9, color: "#c89898", flexShrink: 0 }}>⚠</span>}
                                  <span
                                    onClick={() => setTaskPopup({ taskId: task.id, day })}
                                    style={{ fontSize: 11, color: task.completed ? "#bcb4aa" : overdue ? "#c89898" : "#4c4235", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textDecoration: task.completed ? "line-through" : "none", cursor: "pointer", flex: 1 }}
                                  >{task.title}</span>
                                </div>
          ...
          <div style={{ maxWidth: 500 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {CATEGORIES.map((cat) => (
                  <button key={cat.id} onClick={() => setField("category", cat.id)} style={{ ...chip(newTask.category === cat.id, cat.color), display: "flex", gap: 5, alignItems: "center" }}>{cat.icon} {getCatLabel(cat.id)}</button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: 14 }}>
            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                <div key={cat.id} style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 10, letterSpacing: "0.2em", color: cat.color, textTransform: "uppercase", marginBottom: 7 }}>{cat.icon} {getCatLabel(cat.id)}</div>
                  {catTasks.map((task) => {
                    const dt = getDurType(task.durationType);
